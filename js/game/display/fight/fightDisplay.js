...
FightDisplay.update = display => {
  ...
  // Fight middle
  var middle =
    (player1.collisionBox.pos.x + player1.collisionBox.size.x / 2) / 2 +
    (player2.collisionBox.pos.x + player2.collisionBox.size.x / 2) / 2;
  ...
  if (view.xOffset > fight.stage.size.x - display.canvas.width / display.zoom)
    view.xOffset = fight.stage.size.x - display.canvas.width / display.zoom;
  ...
  [player1, player2].forEach(player => {
    ...
    if (display.debugMode) {
      ...
      display.cx.fillRect(
        player.collisionBox.pos.x * display.zoom,
        player.collisionBox.pos.y * display.zoom,
        player.collisionBox.size.x * display.zoom,
        player.collisionBox.size.y * display.zoom
      );
      ...
      player.hurtboxes.forEach(hurtbox => {
        display.cx.fillRect(
          hurtbox.pos.x * display.zoom,
          hurtbox.pos.y * display.zoom,
          hurtbox.size.x * display.zoom,
          hurtbox.size.y * display.zoom
        );
      });
      ...
      player.hitboxes.forEach(hitbox => {
        display.cx.fillRect(
          hitbox.pos.x * display.zoom,
          hitbox.pos.y * display.zoom,
          hitbox.size.x * display.zoom,
          hitbox.size.y * display.zoom
        );
      });
      ...
      if (player.status) {
        switch (player.status) {
          case "HIT":
            display.cx.drawImage(
              display.assets.c00stun,
              0,
              0,
              91,
              192,
              (Math.floor((Math.random() * player.frame) / 2) -
                player.frame / 4 +
                player.collisionBox.pos.x +
                player.collisionBox.size.x / 2 -
                45) *
                display.zoom,
              (-16 +
                player.collisionBox.pos.y +
                player.collisionBox.size.y / 2 -
                96) *
                display.zoom,
              91 * display.zoom,
              192 * display.zoom
        ...
        switch (player.action) {
          ...
          case "FORWARD_DASH":
            ...
            display.cx.drawImage(
              display.assets.dust,
              128 * Math.floor((player.frame / 2) % 20),
              0,
              128,
              128,
              (-(Math.floor(player.frame / 2) * player.forwardDashSpeed) +
                player.collisionBox.pos.x -
                128) *
                display.zoom,
              (8 +
                player.collisionBox.pos.y +
                player.collisionBox.size.y -
                128) *
                display.zoom,
              128 * display.zoom,
              128 * display.zoom
            ...
            display.cx.drawImage(
              display.assets.dash,
              128 * Math.floor((player.frame / 4) % 20),
              0,
              128,
              128,
          ...
          case "BACKWARD_HIGH":
            display.cx.drawImage(
              display.assets.c00hb,
              91 * Math.floor((display.frame / 8) % 6),
              0,
              91,
              192,
              (player.collisionBox.pos.x +
          ...
          case "AERIAL_A":
            display.cx.drawImage(
              display.assets.c00aa,
              91 * Math.floor((player.frame / 5) % 4),
              0,
              91,
              192,
              (player.collisionBox.pos.x +
          ...
          case "AERIAL_B":
            display.cx.drawImage(
              display.assets.c00ab,
              182 * Math.floor((player.frame / 4) % 6),
              0,
              182,
              192,
              (player.collisionBox.pos.x +
          ...
          case "HIGH_A":
            display.cx.drawImage(
              display.assets.c00ha,
              182 * Math.floor((player.frame / 4) % 4),
              0,
              182,
              192,
              (player.collisionBox.pos.x +
          ...
          case "HIGH_B":
            display.cx.drawImage(
              display.assets.c00hab,
              182 * Math.floor((player.frame / 5) % 6),
              0,
              182,
              192,
              (player.collisionBox.pos.x +
          ...
          case "LOW_A":
            display.cx.drawImage(
              display.assets.c00la,
              182 * Math.floor((player.frame / 3) % 4),
              0,
              182,
              192,
              (player.collisionBox.pos.x +
          ...
          case "QCF":
            display.cx.drawImage(
              display.assets.c00qcf,
              182 * Math.floor((player.frame / 4) % 8),
              0,
              182,
              192,
              (16 +
...
FightDisplay.perspectiveLayer = (display, fight, view) => {
  if (display.assets["s" + fight.stage.id + "l0"]) {
    display.cx.drawImage(
      ...
      0,
      fight.stage.size.x,
      fight.stage.size.y + 16,
      0,
      0,
      fight.stage.size.x * display.zoom,
      (fight.stage.size.y + 16) * display.zoom
    );
  }
  ...
  for (var row = 0; row < img.w; row++) {
    ...
    display.cx.drawImage(
      display.assets["s" + fight.stage.id + "floor"],
      row,
      img.x / 2,
      1,
      img.h,
