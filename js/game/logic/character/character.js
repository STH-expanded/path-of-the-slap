  ...
  constructor(playerId) {
    ...
    this.moveX = game => {
      ...
      var newCollisionBox = new CollisionBox(
        this.collisionBox.pos.plus(new Vector2D(this.speed.x, 0)),
        this.collisionBox.size
      );
      if (!newCollisionBox.isIncludedIn(game.activity.stage))
        newCollisionBox.pos.x =
          newCollisionBox.pos.x < 0
            ? 0
            : game.activity.stage.size.x - newCollisionBox.size.x;

      var other = game.activity.players.find(
        player => player.id !== this.playerId
      ).character;
      if (newCollisionBox.intersects(other.collisionBox)) {
        if (
          other.collisionBox.pos.x === 0 ||
          other.collisionBox.pos.x ===
            game.activity.stage.size.x - other.collisionBox.size.x
        )
          this.speed.x = 0;
        var otherNewCollisionBox = new CollisionBox(
          other.collisionBox.pos.plus(new Vector2D(this.speed.x, 0)),
          other.collisionBox.size
        );

        newCollisionBox.pos.x =
          (otherNewCollisionBox.pos.x + otherNewCollisionBox.size.x / 2 <
          newCollisionBox.pos.x + newCollisionBox.size.x / 2
            ? otherNewCollisionBox.pos.x + otherNewCollisionBox.size.x
            : otherNewCollisionBox.pos.x - newCollisionBox.size.x) +
          other.speed.x;

        [newCollisionBox, otherNewCollisionBox].forEach(collisionBox => {
          if (!collisionBox.isIncludedIn(game.activity.stage))
            collisionBox.pos.x =
              collisionBox.pos.x < 0
                ? 0
                : game.activity.stage.size.x - collisionBox.size.x;
        });
    ...
    this.moveY = game => {
      if (
        this.AERIAL_ACTIONS.includes(this.command) &&
        !this.AERIAL_ATTACKS.includes(this.lastAction)
      )
        this.speed.y -= this.jumpHeight;
      this.speed.y += this.gravity.y;

      let newCollisionBox = new CollisionBox(
        this.collisionBox.pos.plus(new Vector2D(0, this.speed.y)),
        this.collisionBox.size
      );
      if (newCollisionBox.isIncludedIn(game.activity.stage))
        this.collisionBox = newCollisionBox;
    ...
    this.updateDirection = game => {
      ...
      this.direction =
        this.collisionBox.pos.x + this.collisionBox.size.x / 2 ===
    ...
    this.getNewStatus = game => {
      ...
      if (this.status) {
        ...
        var otherHitboxes = other.hitboxes.filter(hitBox =>
          hitBox
            .intersectingCollisionBoxes(this.hurtboxes)
            .some(hurtBox => hurtBox)
        );
    ...
    this.LAND = game => {
      var center = new Vector2D(
        this.collisionBox.pos.x + this.collisionBox.size.x / 2,
        this.collisionBox.pos.y + this.collisionBox.size.y / 2
      );
      this.hurtboxes.push(
        new HurtBox(new Vector2D(center.x, center.y - 4), new Vector2D(50, 106))
      );
    };
    this.GET_UP = game => {};
    ...
    this.BACKWARD_LOW = game => {
      ...
      this.hurtboxes.push(
        new HurtBox(
          new Vector2D(center.x - (this.direction ? 1 : -1) * 4, center.y - 4),
          new Vector2D(54, 72)
        )
      );
    };
    ...
    this.NEUTRAL_LOW = game => {
      ...
      this.hurtboxes.push(
        new HurtBox(new Vector2D(center.x, center.y - 4), new Vector2D(54, 72))
      );
    };
    ...
    this.BACKWARD_HIGH = game => {
      ...
      this.hurtboxes.push(
        new HurtBox(new Vector2D(center.x, center.y - 8), new Vector2D(32, 112))
      );
    };
    ...
    this.NEUTRAL_AERIAL = game => {
      ...
      this.hurtboxes.push(
        new HurtBox(new Vector2D(center.x, center.y - 4), new Vector2D(46, 106))
      );
    };
    ...
    this.LOW_A = game => {
      ...
      if (this.frame > 4 && this.frame < 7) {
        this.hitboxes.push(
          new HitBox(
            new Vector2D(
              center.x + (this.direction ? 1 : -1) * 30,
              center.y - 16
            ),
            new Vector2D(40, 24),
            50,
    ...
    this.HIGH_A = game => {
      ...
      if (this.frame > 6 && this.frame < 9) {
        this.hitboxes.push(
          new HitBox(
            new Vector2D(
              center.x + (this.direction ? 1 : -1) * 32,
              center.y - 24
            ),
            new Vector2D(40, 24),
            50,
    ...
    this.HIGH_B = game => {
      ...
      if (this.frame > 11 && this.frame < 18) {
        this.hitboxes.push(
          new HitBox(
            new Vector2D(
              center.x + (this.direction ? 1 : -1) * 48,
              center.y - 16
            ),
            new Vector2D(72, 32),
            75,
    ...
    this.getCommandInput = (game, inputList) => {
      ...
      if (
        !this.runDash &&
        this.action === "FORWARD_DASH" &&
        this.frame < this.forwardDashFrame
      )
        return "FORWARD_DASH";

      if (
        this.collisionBox.pos.y + this.collisionBox.size.y >=
          game.activity.stage.size.y &&
        this.AERIAL_FULL.includes(this.action)
      ) {
        return "LAND";
        ...
        ((inputs.a &&
          ...
          !inputs.left &&
          inputList.length > 2 &&
          inputList[inputList.length - 2].frames < 8 &&
          this.GROUND_ACTIONS.includes(this.action) &&
          !this.DASH_ACTIONS.includes(this.action) &&
          ((inputList[inputList.length - 2].inputs.right &&
            !inputList[inputList.length - 2].inputs.down &&
            inputList[inputList.length - 3].inputs.down) ||
            (inputs.right &&
              inputList[inputList.length - 2].inputs.right &&
              inputList[inputList.length - 3].inputs.down) ||
            (inputs.right &&
        ...
        ((inputs.a &&
          ...
          this.GROUND_ACTIONS.includes(this.action) &&
          !this.DASH_ACTIONS.includes(this.action) &&
          ((inputList[inputList.length - 2].inputs.left &&
            !inputList[inputList.length - 2].inputs.down &&
            inputList[inputList.length - 3].inputs.down) ||
            (inputs.left &&
              inputList[inputList.length - 2].inputs.left &&
        ...
        ((inputs.a && this.HIGH_ACTIONS.includes(this.action)) ||
          this.action === "HIGH_A") &&
        this.frame < this.highAFrame
      ) {
        return "HIGH_A";
      } else if (
        ((inputs.b && this.HIGH_ACTIONS.includes(this.action)) ||
          this.action === "HIGH_B") &&
        this.frame < this.highBFrame
      ) {
        return "HIGH_B";
      } else if (
        ((inputs.a && this.LOW_ACTIONS.includes(this.action)) ||
          this.action === "LOW_A") &&
        this.frame < this.lowAFrame
      ) {
        return "LOW_A";
      } else if (
        ((inputs.b && this.LOW_ACTIONS.includes(this.action)) ||
          this.action === "LOW_B") &&
        this.frame < this.lowBFrame
      ) {
        return "LOW_B";
      } else if (
        ((inputs.a && this.AERIAL_ACTIONS.includes(this.action)) ||
          this.action === "AERIAL_A") &&
        this.frame < this.aerialAFrame
      ) {
        return "AERIAL_A";
      } else if (
        ((inputs.b && this.AERIAL_ACTIONS.includes(this.action)) ||
          this.action === "AERIAL_B") &&
        this.frame < this.aerialBFrame
      ) {
        return "AERIAL_B";
      } else if (
        (inputs.up && this.GROUND_ACTIONS.includes(this.action)) ||
        this.AERIAL_ATTACKS.includes(this.action)
      ) {
        if (
          (inputs.left && !this.direction) ||
          (inputs.right && this.direction) ||
          (this.AERIAL_ATTACKS.includes(this.action) &&
            this.lastAction === "FORWARD_AERIAL")
        ) {
          ...
          (inputs.right && !this.direction) ||
          (this.AERIAL_ATTACKS.includes(this.action) &&
            this.lastAction === "BACKWARD_AERIAL")
        ) {
        ...
        ((this.runDash && inputs.right) ||
          (!this.runDash &&
            inputs.right &&
            this.lastAction !== "FORWARD_DASH" &&
            this.action !== "FORWARD_DASH")) &&
        this.direction &&
        (this.action === "FORWARD_DASH" ||
          (this.action === "NEUTRAL_HIGH" &&
            inputList.length > 2 &&
            !inputList[inputList.length - 2].inputs.down &&
            !inputList[inputList.length - 2].inputs.up &&
            !inputList[inputList.length - 2].inputs.a &&
            !inputList[inputList.length - 2].inputs.b &&
            inputList[inputList.length - 2].frames < 8 &&
            inputList[inputList.length - 3].inputs.right))
      ) {
        return "FORWARD_DASH";
      } else if (
        ((this.runDash && inputs.left) ||
          (!this.runDash &&
            inputs.left &&
            this.lastAction !== "FORWARD_DASH" &&
            this.action !== "FORWARD_DASH")) &&
        ...
        (this.action === "FORWARD_DASH" ||
          (this.action === "NEUTRAL_HIGH" &&
            ...
            !inputList[inputList.length - 2].inputs.b &&
            inputList[inputList.length - 2].frames < 8 &&
            inputList[inputList.length - 3].inputs.left))
      ) {
        return "FORWARD_DASH";
      } else if (
        inputs.left &&
        this.direction &&
        (this.action === "BACKWARD_DASH" ||
          (this.action === "NEUTRAL_HIGH" &&
            inputList.length > 2 &&
            !inputList[inputList.length - 2].inputs.down &&
            !inputList[inputList.length - 2].inputs.up &&
    ...
    this.update = (game, inputList) => {
      ...
      if (
        !this.AERIAL_FULL.includes(this.action) &&
        !this.LAG_ACTIONS.includes(this.action)
      )
        this.updateDirection(game);
