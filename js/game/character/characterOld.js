class Character {
    constructor(action, direction, position) {
        ...
        this.moveX = (game) => {
            ...
            if (!newCollisionBox.includedInAxis(game.activity.stage.collisionBox, "x")) newCollisionBox.pos.x = newCollisionBox.pos.x < 0 ? 0 : game.activity.stage.collisionBox.size.x - newCollisionBox.size.x;
            ...
            if (newCollisionBox.intersects(other.collisionBox)) {
                if (other.collisionBox.pos.x === 0 || other.collisionBox.pos.x === game.activity.stage.collisionBox.size.x - other.collisionBox.size.x) this.speed.x = 0;
                ...
                [newCollisionBox, otherNewCollisionBox].forEach((collisionBox) => {
                    if (!collisionBox.includedInAxis(game.activity.stage.collisionBox, "x")) collisionBox.pos.x = collisionBox.pos.x < 0 ? 0 : game.activity.stage.collisionBox.size.x - collisionBox.size.x;
                });
        ...
        this.moveY = (game) => {
            ...
            if (newCollisionBox.pos.y + newCollisionBox.size.y <= game.activity.stage.collisionBox.pos.y + game.activity.stage.collisionBox.size.y) this.collisionBox = newCollisionBox;
            else this.speed.y = 0;
        };
        ...
        this.getNewStatus = (game, inputList) => {
            ...
            if (this.status) {
                if (!this.frame) {
                            ...
                            if ((!other.HIGH_ATTACKS.includes(other.action) && this.action === 'BLOCK_HIGH') || (!other.LOW_ATTACKS.includes(other.action) && this.action === 'BLOCK_LOW')) {
                                ...
                                if (hitBox.status === false) {
                                    ...
                                    this.knockbacks.push({
                                        affection: (this.collisionBox.pos.x <= 0 || this.collisionBox.pos.x + this.collisionBox.size.x >= game.activity.stage.collisionBox.size.x) && hitBox.type === 'storke' ? 'launcher' : 'self',
                    ...
                    if (this.status === 'EJECTED' && this.collisionBox.pos.y + this.collisionBox.size.y === game.activity.stage.collisionBox.size.y) {
                        newStatus = 'GROUND';
                        this.frame = 1;
                    }
                    ...
                    this.knockbacks.forEach((knockback) => {
                        ...
                        if (knockback.affection === 'launcher') {
                            ...
                            if (!newCollisionBox.includedInAxis(game.activity.stage.collisionBox, "x")) {
                                if (newCollisionBox.pos.x < 0) {
                                    newCollisionBox.pos.x = 0;
                                } else {
                                    newCollisionBox.pos.x = game.activity.stage.collisionBox.size.x - newCollisionBox.size.x;
                                }
                            }
                            this.collisionBox = newCollisionBox;
                        }
                ...
                game.activity.actors.forEach((projectile) => {
        ...
        this.getCommandInput = (game, inputList) => {
            ...
            if (this.collisionBox.pos.y + this.collisionBox.size.y >= game.activity.stage.collisionBox.size.y && this.AERIAL_FULL.includes(this.action)) {
                return 'LAND';
