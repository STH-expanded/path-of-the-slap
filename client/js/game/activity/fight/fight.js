class Fight extends Activity {
	...
	initRound = () => {
		...
		this.players.forEach((player, index) => {
			player.resetInput();
			player.character = new Character(player.selectedCharacter, this.winCount.filter((element)=>element == 0).length == 2  ? "INTRO" : "IDLE", !index % 2, new Vector2D(Math.floor((this.stage.collisionBox.size.x / 3) * (1 + index)), this.stage.collisionBox.size.y));
		});
	...
	update = game => {
		...
		// Update players
		const enablePause = this.players.findIndex((player) => player instanceof Online) === -1
		this.players.forEach((player,index) => {
				// Update characters
				if (!this.pauseMenu) {
				...
				if (player !== game.computer  && !player.character.actions[player.character.action].disableMenu===true) {
					...
					if (enablePause) this.pauseMenu = currentInput.start && !lastInput.start ? new PauseMenu(0, 0, ["Resume", "Rematch", "CharacterSelection", "MainMenu"], 2, this) : this.pauseMenu;	
				}
		});
		// Update actors
